...
fetch("https://api.github.com/users/Meghaanil")
.then(res=>{
    return res.json()
})
.then(data=>{
...
async function basic_info_get(){
    const res = await fetch('https://api.github.com/users/Meghaanil');
...
async function repos_info_get(){
    ...
    const res = await fetch('https://api.github.com/users/Meghaanil/repos');
    const data = await res.json()
